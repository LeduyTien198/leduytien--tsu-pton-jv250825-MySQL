...
AS
SELECT e.EmployeeID, e.FullName, e.Position, BranchName, Location
...
SELECT EmployeeID, FullName, Position, Salary, HireDate
FROM Employees WHERE Salary >= 15000000
WITH CHECK OPTION;
...
SELECT e.EmployeeID, e.FullName, e.Position, BranchName, Location, PhoneNumber
FROM Employees e JOIN Branch  b
ON e.BranchID = b.BranchID;
select * from EmployeeBranch;
...
from Branch br
join Employees em 
on br.BranchID = em.BranchID
where br.BranchName = 'Chi nhánh Hà Nội';
select * from EmployeeBranch;
...
create index index_PhoneNumber on Customers(PhoneNumber);
...
create index CompositeIndex on Employees(BranchID,Salary);
explain select * from Employees where BranchID = 3 ;
...
DROP INDEX index_PhoneNumber ON Customers;
DROP INDEX CompositeIndex ON Employees; -- có fk thì ko nên xóa...
...
DELIMITER $$
CREATE PROCEDURE GetCustomerByPhone(IN p_Phone VARCHAR(15))
BEGIN
    SELECT *
    FROM Customers
...
-- GetTotalBalance: Tạo một thủ tục nhận tham số IN là CustomerID và sử dụng tham số OUT để 
-- trả về tổng số dư của tất cả tài khoản của khách hàng đó. 
drop procedure if exists GetTotalBalance;
delimiter $$

create procedure GetTotalBalance(in _CustomerID int, out _total decimal(10,2))
	
begin
	select  ifnull(sum(Balance), 0) into _total
    from accounts where CustomerID = _CustomerID;
end$$
	
delimiter ;

set @total = 0;
call GetTotalBalance(2, @total);
select @total;
...
-- Thủ tục cũng nhận tham số IN là EmployeeID để cập nhật mức lương mới vào bảng Employees.

drop procedure if exists IncreaseEmployeeSalary;
delimiter $$

create procedure IncreaseEmployeeSalary(in _EmployeeID int, inout _currunSalary decimal(10,2))
	
begin
	set _currunSalary = _currunSalary * 1.10;
    update Employees set Salary = _currunSalary 
    where EmployeeID = _EmployeeID;
end$$
	
delimiter ;
call IncreaseEmployeeSalary(4, @cr_salary);
select Salary into @cr_salary from Employees where EmployeeID = 4;
select @cr_salary as 'luong sau khi tang';
select * from Employees;

-- --------------------------------------------------------------------------------------------------------------------------------------------
-- THỰC HÀNH 4

-- GetCustomerByPhone: Tạo một thủ tục nhận tham số IN 
-- là số điện thoại để tìm kiếm và trả về thông tin chi tiết của khách hàng.
drop procedure if exists GetCustomerByPhone;
delimiter $$
create procedure GetCustomerByPhone(in _PhoneNumber varchar(15))
begin
	select * from customers where  PhoneNumber = _PhoneNumber;
end $$
delimiter ;
call GetCustomerByPhone('0901234567');
-- GetTotalBalance: Tạo một thủ tục nhận tham số IN là CustomerID 
-- và sử dụng tham số OUT để trả về tổng số dư của tất cả tài khoản của khách hàng đó.
drop procedure if exists GetTotalBalance;
delimiter $$
create procedure GetTotalBalance(in _CustomerID int, out _Balance decimal(15,2))
begin
	select ifnull(sum(Balance), 0) into _Balance
    from accounts where CustomerID = _CustomerID;
end //
delimiter ;
SET @total = 0;

-- Lấy tổng số dư của khách hàng ID = 2
SELECT IFNULL(SUM(Balance), 0) INTO @total
FROM Accounts
WHERE CustomerID = 2;
-- Hiển thị kết quả
SELECT @total AS 'tong';
