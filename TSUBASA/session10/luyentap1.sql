...
BEGIN
    ...
    IF OLD.quantity > 10 THEN
        SIGNAL SQLSTATE  '45000' SET message_text = CONCAT('ko the xoa ');
    END IF;
...
-- Test trigger
-- Giả sử sản phẩm có ID = 1 có quantity = 15
SELECT * FROM products WHERE productID = 1;

-- Câu lệnh này sẽ kích hoạt trigger
DELETE FROM products WHERE productID = 1;
-- Nếu quantity > 10, bạn sẽ nhận được thông báo lỗi từ trigger

-- Kiểm tra với sản phẩm có quantity <= 10
DELETE FROM products WHERE productID = 2; -- Nếu quantity <= 10, sẽ xóa thành công
